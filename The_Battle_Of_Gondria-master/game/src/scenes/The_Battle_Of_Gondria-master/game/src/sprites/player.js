export default class Player {
  ...
  interaction(alavanca, ponte, aldeao, casa) {
    ...
    if (casa != null) {
      ...
      let distanciaCasaY = casa.y - this.sprite.body.y;
      if (this.hasIntoHause == false) {

        if (distanciaCasaY <= 64) {
          if ((distanciaCasaX < 20) && (distanciaCasaX > -16) && (this.chave == 1)) {
            this.hasIntoHause = true;
            this.scene.scene.start('Level_casa', { player: this });
          }
        }
      } else {
        alert('qasqsq');
  ...
  updateHUD() {
    ...
    this.scene.scoreLabel.text = this.score;
